...
const articlesData: Record<string, any> = {
  "1": {
    ...
    content: `
      ...
      <p class="mb-6 leading-relaxed">Sering kali, gejala awal osteosarcoma dianggap remeh sebagai cedera olahraga ringan or "growing pains" (nyeri pertumbuhan). Perhatikan tanda-tanda berikut:</p>
...
function ArticleDetailContent() {
  ...
  useEffect(() => {
    ...
    // A. Check local fallback articles
    if (articlesData[id]) {
    ...
    async function fetchArticle() {
      try {
        ...
        // C. If remote search fails, try searching the general list
        ...
        if (cfListRes.ok) {
          ...
          if (data?.articles && Array.isArray(data.articles)) {
            ...
            if (matchedFromList) {
              ...
              if (cfDetailRes.ok) {
                const matchedDetail = await cfDetailRes.json();
                if (matchedDetail && !matchedDetail.error) {
                  setArticle({
